...
function processSum(numberList, callback) {
  let numSum = numberList.reduce((a, b) => { return a + b }, 0);
...
function processContains(item, list, callback) {
  return callback(isItContain(item, list));
}

function isItContain(item, list) {
  let newBoo = list.includes(item);
  return newBoo;
}
/**
...
function getFullNames(runners) {
  /* CODE HERE */
  let newArray = [];
  runners.forEach((array) => {
    let fullName = array.last_name + ", " + array.first_name;
...
function firstNamesAllCaps(runners) {
  ...
  let firstAllCaps = [];
  runners.map((array) => {
    firstAllCaps.push(array.first_name.toUpperCase());
  })
  return firstAllCaps;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  /* CODE HERE */
  let newArray = [];
  runners.filter((array) => {
    if (array.shirt_size === tShirtSize) {
      newArray.push(array)
    }
  })
  return newArray;
